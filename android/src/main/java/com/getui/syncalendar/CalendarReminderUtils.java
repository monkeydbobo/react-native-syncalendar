...
public class CalendarReminderUtils {
    ...
    private static String CALENDARS_NAME = "admin";
    private static String CALENDARS_ACCOUNT_NAME = "admin@baobao.com";
    private static String CALENDARS_ACCOUNT_TYPE = "com.baobao.admin";
    private static String CALENDARS_DISPLAY_NAME = "admin账户";

    private static Context context;


    public CalendarReminderUtils(Context mContext) {
        context = mContext;
    }
     ...
     * 100: '系统版本太低，不支持设置日历',
     * 101: '日历打开失败，请稍后重试',
     * 102: '不允许使用日历,请在设置中允许此App使用日历',
     * 103: '已同步',
     * 104: '参数错误',
     * 200: '会议添加成功',
     */
    public static String addCalendarEvent(String id, String title, String location, long reminderTime, long endTime, ReadableArray previousDate) {
        ...
        if (previousDate == null || previousDate.size() == 0) {
            Log.d(TAG, "previousDate is empty or invalid");
            return "200";
        }
        //事件提醒的设定
        for (int i = 0; i < previousDate.size(); i++) {
            ContentValues values = new ContentValues();
            values.put(CalendarContract.Reminders.EVENT_ID, ContentUris.parseId(newEvent));
            values.put(CalendarContract.Reminders.MINUTES, previousDate.getInt(i));// 提前previousDate天有提醒
